...
module.exports = function (config) {
  ...
  module.resetPhoneIDVerificationByUid = function (
    uid,
    smsTemplate,
    isVoiceOtp
  ) {
  ...
  module.revokeAllRefreshToken = function (uid) {
    if (helper.isNullOrWhiteSpace(uid)) {
      return Promise.reject(helper.getValidationMessage('uid'));
    }
    var queryParameters = {};
  ...
  module.multipurposeEmailTokenGeneration = function (
    multiEmailToken,
    tokentype
  ) {
    if (helper.checkJson(multiEmailToken)) {
      return Promise.reject(helper.getValidationMessage('multiEmailToken'));
    }
    if (helper.isNullOrWhiteSpace(tokentype)) {
      return Promise.reject(helper.getValidationMessage('tokentype'));
    }
    var queryParameters = {};
  ...
  module.multipurposeSMSOTPGeneration = function (multiSmsOtp, smsotptype) {
    if (helper.checkJson(multiSmsOtp)) {
      return Promise.reject(helper.getValidationMessage('multiSmsOtp'));
    }
    if (helper.isNullOrWhiteSpace(smsotptype)) {
      return Promise.reject(helper.getValidationMessage('smsotptype'));
    }
    var queryParameters = {};
    ...
    return config.request('POST', resourcePath, queryParameters, multiSmsOtp);
  };
