...
```typescript
async function callQueryUserProfilesInsights(client: LoginRadius) {
  const requestPayload = {  }; //Required
  const region: string = '<region>'; //Optional

  try {
    const res = await client.<service>.queryUserProfilesInsights({ requestPayload, region });
